...
try:
    ...
    for i in range(len(boards)):
        logger.debug("Publishing to %s", pubTopics[i])                                 
        mqttc.publish(pubTopics[i], boards[i].readRelays(), qos=mqttQos,retain=mqttRetain)
        ...
        time.sleep(1)
        logger.debug("Turning second on...")
        boards[i].turnOnRelay(2)

        # time.sleep(1)
        # logger.debug("Turning second off...")
        # boards[i].turnOffRelay(2)

        # time.sleep(1)
        # logger.debug("Turning on all...")
        # boards[i].turnAllOn()

        mqttc.publish(pubTopics[i], boards[i].readRelays(), qos=mqttQos,retain=mqttRetain)

        time.sleep(1)
        logger.debug("Turning off all...")
        boards[i].turnAllOff()
